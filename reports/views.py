...
def owner_dashboard(request):
    ...
    # TOPlar (amount bo‘yicha)
    top_stores = list(base.values("store__name").annotate(s=Sum("amount")).order_by("-s")[:5])
    top_brands = list(base.values("product__brand__name").annotate(s=Sum("amount")).order_by("-s")[:5])
    top_models = list(
        base.values("product__brand__name", "product__model__name").annotate(s=Sum("amount")).order_by("-s")[:5]
    )
